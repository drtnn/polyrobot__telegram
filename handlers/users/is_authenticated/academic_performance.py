...
from utils.polyrobot.utils import select_semester_message_text, select_lesson_message_text


@dp.message_handler(commands=["performance"], is_authenticated=True)
async def bot_academic_performance_command(message: Message):
    ...
    await message.answer(text=select_semester_message_text(),
                         reply_markup=select_semester_buttons(semesters_count))


@dp.message_handler(text=ACADEMIC_PERFORMANCE_BUTTON, is_authenticated=True)
async def bot_academic_performance_text(message: Message):
    await bot_academic_performance_command(message)


@dp.callback_query_handler(academic_performance.filter())
async def bot_academic_performance_callback(call: CallbackQuery, callback_data: dict):
    ...
    await call.message.edit_text(text=select_semester_message_text(),
                                 reply_markup=select_semester_buttons(semesters_count))


@dp.callback_query_handler(academic_performance_semester.filter())
async def bot_academic_performance_semester_callback(call: CallbackQuery, callback_data: dict):
    ...
    if academic_performances:
        await call.message.edit_text(
            text=select_lesson_message_text(academic_performances),
...
@dp.callback_query_handler(academic_performance_lesson.filter())
async def bot_academic_performance_lesson_callback(call: CallbackQuery, callback_data: dict):
    ...
    academic_performance = [academic_performance for academic_performance in academic_performances
                            if academic_performance.id == callback_data['lesson_id']][0]
